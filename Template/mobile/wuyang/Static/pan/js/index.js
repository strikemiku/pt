$(function () {
    var tips = ["50资产", "5资产", "35资产", "20资产", "10资产","谢谢参与~"],//中奖提示
        $ring = $(".ring"),
        $prize = $(".prize"),//转盘
        $btn = $("#btn"),//按钮
        $num = '',
    ...
    $btn.click(function () {
        ...
        $.ajax({
            type : 'post',
            url :"/Mobile/User/check_luck_num",
            data : {},
            dataType : 'json',
            success : function(res){
                data.count = res.chou_num;
                if (data.count<=0) { //当抽奖次数为0时
                    $change.html(0);//次数显示为0
                    bool = false;
                    layer.msg("今日还有0次抽奖机会");
                    return false;
                } else { //还有次数就执行
                    data.count--;
            ...
            success : function(res){
                if(res.status == 200){
                    switch (data) {//中奖概率，可控。根据得到的随机数控制奖品
                        case 1:
                            rotateFn(1, 0, "<b>恭喜您</b><br/>抽中了8元红包"); // 8元红包
                            break;
                        case 2:
                            rotateFn(2, -60, "<b>恭喜您</b><br/>抽中了5元红包"); // 5元红包
                            break;
                        case 3:
                            rotateFn(3, -120, "<b>恭喜您</b><br/>抽中了2元红包"); // 2元红包
                            break;
                        case 4:
                            rotateFn(4, -180, "<b>恭喜您</b><br/>抽中了10元红包"); //10元红包
                            break;
                        case 5:
                            rotateFn(5, -240, "<b>恭喜您</b><br/>抽中了1元红包"); // 1元红包
                            break;
                        case 6:
                            rotateFn(6,  60, "<b>很遗憾</b><br/>谢谢参与");
                            break;
                    }
                    $change.html(count);//显示剩余次数
                }else{
                    $change.html(count);//显示剩余次数
                    layer.msg(res.msg);
                    return false;
