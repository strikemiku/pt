...
//        dump($goods_category_tree);die;
    ...
    public function list2(){
        $where['type'] = 2;
        $where['is_on_sale'] = 1;
        $jifen_list = M("goods")->where($where)->limit(20)->field("goods_id,goods_name,shop_price,market_price,original_img,keywords")->order("sort desc")->select();
        $this->assign('list',$jifen_list);
        $this->display();
    }
 /**
     * 商品详情页
    ...
    public function goodsInfo(){
        ...
        if($user){
            ...
            $user['level_pt_num'] = M("user_level")->where("level=".$user['level'])->getField("pt_num");
        }
        $this->assign("user",$user);
        $goods_images_list = M('GoodsImages')->where("goods_id = $goods_id")->select(); // 商品 图册        
        $goods_attribute = M('GoodsAttribute')->getField('attr_id,attr_name'); // 查询属性
        ...
        $goods['pz_num'] = M("home_order")->where("status=2 and goods_id=".$goods['goods_id'])->count();
        $goods['total_pt_num'] = M("home_order")->where("status=1 and goods_id=".$goods['goods_id'])->count();
        $goods['chou_num'] = $group_num['num']*($group_num['award_bl']/100);
        $pt_list = M("home")->where("status=1 and goods_id=".$goods['goods_id'])->order("id desc")->select();
        if($pt_list){
            foreach ($pt_list as $kk=>$vv){
                $home_order= M("home_order")->where("home_id=".$vv['id'])->field("user_id")->order("id asc")->select();
                foreach ($home_order as $k=>$v){
                    $home_order[$k]['user_pic'] = M("users")->where("user_id=".$v['user_id'])->getField("head_pic");
                }
                $pt_list[$kk]['home_code_show']=substr($vv['home_code'],6);
                $pt_list[$kk]['orders'] = $home_order;
            }
        }
        //为你推荐
        $tjList = M("goods")->where("is_on_sale=1 and group_price=".$goods['group_price']." and goods_id !=".$goods['goods_id'])->limit(4)->order("sort asc")->field("goods_id,goods_name,original_img,keywords,shop_price,sales_sum,group_num,market_price")->select();
        if($tjList){
            foreach ($tjList as $kk=>$vv){
                $tjList[$kk]['sq_money'] = $vv['market_price']-$vv['shop_price'];
            }
        }
        $this->assign('tjList',$tjList);
        $this->assign('config',$config);
        $this->assign('ptList',$pt_list);
        $this->assign('goods_attribute',$goods_attribute);//属性值     
        $this->assign('goods_attr_list',$goods_attr_list);//属性列表
        $this->assign('filter_spec',$filter_spec);//规格参数
        $this->assign('goods_images_list',$goods_images_list);//商品缩略图
