...
class OrderLogic extends RelationModel
{
    ...
    public function getOrderInfo($order_id)
    {
        ...
        $order['user_name'] = M("users")->where("user_id=".$order['user_id'])->getField("nickname");
...
//        	$btn['refund'] = '申请退货';
    ...
    public function deliveryHandle($data){
        $shipping_code = $data['shipping'];// 物流方式
        $shipping_name= M('plugin')->where(array('status'=>1,'type'=>'shipping','code'=>$data['shipping']))->getField('name');
        ...
        $updata['shipping_name'] = $shipping_name;
