...
class OrderController extends BaseController {
    ...
    public function maichu()
    {
        ...
        foreach ($list as $kk=>$vv){
            if($vv['order_id']==0){
                $list[$kk]['order_sn'] = date('YmdHis').rand(1000,9999);
            }else{
                $order_info = M("order")->where("order_id=".$vv['order_id'])->field("order_id,order_sn")->find();
                $list[$kk]['order_sn'] = $order_info['order_sn'];
            }
            $list[$kk]['goods_name'] = M("goods")->where("goods_id=".$vv['goods_id'])->getField('goods_name');
            if($vv['sell_user_id']!=0){
                $sell_user = M("users")->where(array('user_id'=>$vv['sell_user_id']))->field("user_id,nickname,mobile")->find();
                $list[$kk]['sell_name'] = $sell_user['nickname'];
            }else{
                $list[$kk]['sell_name'] = "admin";
            }
            if($vv['buy_user_id']){
                $buy_user = M("users")->where(array('user_id'=>$vv['buy_user_id']))->field("user_id,nickname,mobile")->find();
                $list[$kk]['buy_name'] = $buy_user['nickname'];
                $list[$kk]['buy_mobile'] = $buy_user['mobile'];
            }
        }
        $show  = $Page->show();
    ...
    public function ajaxindex(){
        ...
        foreach ($orderList as $kk=>$vv){
            $orderList[$kk]['goods_name'] = M("goods")->where("goods_id=".$vv['goods_id'])->getField("goods_name");
            $orderList[$kk]['nickname'] = M("users")->where("user_id=".$vv['user_id'])->getField("nickname");
        }
    ...
    public function ajaxputong(){
        ...
        I('order_sn') ? $condition['order_sn'] = trim(I('order_sn')) : false;
        I('order_status') != '' ? $condition['order_status'] = I('order_status') : false;
        I('sh_status') != '' ? $condition['sh_status'] = I('sh_status') : false;
        I('pay_status') != '' ? $condition['pay_status'] = I('pay_status') : false;
        I('pay_code') != '' ? $condition['pay_code'] = I('pay_code') : false;
        I('shipping_status') != '' ? $condition['shipping_status'] = I('shipping_status') : false;
        I('user_id') ? $condition['user_id'] = trim(I('user_id')) : false;
    ...
    public  function dd_identify(){
        ...
        if(I('status')==1){  //审核通过
            ...
            update_number($order_info['order_id'],$order_info['user_id']);
            $orderGoodsArr = M('OrderGoods')->where("order_id = ".$order_info['order_id'])->field("goods_id,goods_name,goods_num")->select();
            foreach($orderGoodsArr as $key => $val)
            {
                $goods = M("goods")->where("goods_id=".$val['goods_id'])->field("goods_id,shop_price,yunfei,is_recommend")->find();
                $award_money = ($goods['shop_price']+$goods['yunfei'])*$val['goods_num'];
                if($goods['is_recommend']==1){
                    //极差奖
                    jicha_award_new($order_info['user_id'],$award_money);
                }
            }
        }else{
            //审核拒绝退还积分
            if($order_info['order_jifen'] > 0){
                //更新用户的积分
                M("users")->where("user_id=".$order_info['user_id'])->setInc("pay_points",$order_info['order_jifen']);
                upd_jifen($order_info['user_id'],$order_info['order_jifen'],1,"订单审核未通过退还积分",5);
            }
            $info = array(
                'sh_miaoshu' => I('remark'),
                'sh_status' => I('status'),
        ...
        $model->where(array('order_id'=>$id))->save($info);
        $this->ajaxReturn(array('msg'=>'审核成功','url'=>U('Order/putong'),'state'=>200));
    }
    /*
     * ajax 发货订单列表
    */
...
//    	$condition['shipping_status'] = $shipping_status ? $shipping_status : array('in',[0,1,2]);
    	...
    	foreach ($orderList as $kk=>$vv){
            $orderList[$kk]['goods_name'] = M("goods")->where("goods_id=".$vv['goods_id'])->getField("goods_name");
            $orderList[$kk]['nickname'] = M("users")->where("user_id=".$vv['user_id'])->getField("nickname");
            $orderList[$kk]['invoice_no']=M('delivery_doc')->where('order_id='.$vv['order_id'])->getField("invoice_no");
        }
    	$this->assign('orderList',$orderList);
    	$this->assign('page',$show);// 赋值分页输出
    	$this->display();
    ...
    public function delivery_info(){
        ...
        $order['p_name'] = M('region')->where('id='.$order['province'])->getField('name');
        $order['c_name'] = M('region')->where('id='.$order['city'])->getField('name');
        $order['d_name'] = M('region')->where('id='.$order['district'])->getField('name');
        //获取配送方式
        $shipping_list = M('plugin')->where(array('status'=>1,'type'=>'shipping'))->select();
        $this->assign('shipping_list',$shipping_list);
		$this->assign('order',$order);
