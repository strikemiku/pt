...
//			exit($file);
    ...
    public function initialize(){
        if(IS_POST){
            M("admin_log")->where("1= 1")->delete();
            M("chongzhi")->where("1=1")->delete();
            M("cart")->where("1=1")->delete();
            M("order")->where("1=1")->delete();
            M("home")->where("1=1")->delete();
            M("home_order")->where("1=1")->delete();
            M("order_goods")->where("1=1")->delete();
            M("order_action")->where("1=1")->delete();
            M("update_level")->where("1=1")->delete();
            M("game_log")->where("1=1")->delete();
            M("user_award")->where("1=1")->delete();
            M("user_back")->where("1=1")->delete();
            M("user_active")->where("1=1")->delete();
            M("user_devote")->where("1=1")->delete();
            M("user_exchange")->where("1=1")->delete();
            M("user_recom")->where("1=1")->delete();
            M("user_sign")->where("1=1")->delete();
            M("user_tx")->where("1=1")->delete();
            M("user_jifen")->where("1=1")->delete();
            M("user_money")->where("1=1")->delete();
            M("user_address")->where("1=1")->delete();
            M("user_relation")->where("1=1")->delete();
            M("withdrawals")->where("1= 1")->delete();
            M("users")->where("user_id!=1")->delete();
            $data['level'] = 1;
            $data['type'] = 0;
            $data['user_money'] = 0;
            $data['pay_points'] = 0;
            ...
            $data['gda_num'] = 0;
            $data['gdb_num'] = 0;
            M("users")->where("user_id = 1")->save($data);
            $this->ajaxReturn(array('msg'=>'数据初始化成功','url'=>U('Tools/initialize'),'status'=>200));
        }
        $this->display();
    }
}