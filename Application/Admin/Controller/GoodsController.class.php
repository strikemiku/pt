...
class GoodsController extends BaseController {
    ...
    public function set(){
        ...
        M("goods")->where("1=1")->save(array('is_member'=>0));
        M("goods")->where("goods_id='$goods_id'")->save(array('is_member'=>1));
    ...
    * */
    public function set_goods(){
        ...
        $data['goods_num'] =1;
        $data['maichu_money'] =$goods['shop_price']*($config['basic_sell_bl']/100);
        $data['order_status'] =1;
        $data['type'] =2;
        $data['is_tui'] =0;
        $data['add_time'] =time();
    ...
    public function delGoodsCategory(){
        ...
        if($count>0){
            $this->ajaxReturn(array('status'=>300,'msg'=>'该分类下还有子分类不得删除!'));
        }
        // 判断是否存在商品
        $goods_count = M('Goods')->where("cat_id = {$_GET['id']}")->count('1');
        if($goods_count>0){
            $this->ajaxReturn(array('status'=>300,'msg'=>'该分类下有商品不得删除!'));
        }
        // 删除分类
...
//        var_dump($goodsList);die;
     ...
     * 添加修改商品
     */
    public function addEditfxGoods()
    {
        $GoodsLogic = new GoodsLogic();
        $Goods = D('Goods'); //
        ...
        if (($_GET['is_ajax'] == 1) && IS_POST) {
            ...
            if (!$Goods->create(NULL, $type))// 根据表单提交的POST数据创建数据对象
            {
                ...
                $return_arr = array(
                    'status' => 1,
                    'msg' => '操作成功',
                    'data' => array('url' => U('Admin/Goods/fxList')),
                );
                $this->ajaxReturn(json_encode($return_arr));
            }
        }
        ...
        $group_price =M('group_price')->where("is_show=1")->order("id asc")->select();
        $this->assign('group_price',$group_price);
        $bd_level =M('user_level')->where("level>=2 and level <=4")->order("level asc")->select();
        $this->assign('bd_level',$bd_level);
    ...
    public function addEditGoods()
    {
        ...
        if (($_GET['is_ajax'] == 1) && IS_POST) {
            ...
            if($_POST['group_num']>0){
                $_POST['num_id'] = M("group_num")->where("num=".$_POST['group_num'])->getField("id");
            }
            if($_POST['group_price']>0){
                $_POST['price_id'] = M("group_price")->where("price=".$_POST['group_price'])->getField("id");
            }
            if (!$Goods->create(NULL, $type))// 根据表单提交的POST数据创建数据对象
            {
                //  编辑
