...
//发送短信验证码
function send_sms_mobile($mobile){
    $config = getConfig();
    $yzm =  mt_rand(1000,9999);//验证码
    $url = "http://sms.izjun.cn/v2sms.aspx";
  
    $data = [];
    $data['action'] = 'send';
    $data['userid'] = $config['sms_sms_id'];
    $data['timestamp'] = date("YmdHis",time());
    $data['sign'] = MD5($config['sms_sms_name'].$config['sms_sms_key'].$data['timestamp']);
    $data['mobile'] = $mobile;
    $data['content'] = "【趣购好物】:您的验证码是".$yzm.".如非本人操作,请勿略本短信";
    $data['sendTime'] = "";
    $data['extno'] = "";
    send_post($url,$data);
    return $yzm;
